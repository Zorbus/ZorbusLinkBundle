...
class BlockCategoriesConfig extends BlockConfig
{

    public function __construct(AdminInterface $admin, FormFactory $formFactory, $httpKernel)
    {
        parent::__construct('zorbus_link.block.categories', 'Categories Block', $admin, $formFactory);
        $this->enabled = true;
        $this->themes = array(
            'ZorbusLinkBundle:Block:categories' => 'Default'
            );
    ...
    public function getBlockEntity(array $data, BlockEntity $block = null)
    {
        ...
        $block->setParameters(json_encode(array('category_ids' => $categories)));
